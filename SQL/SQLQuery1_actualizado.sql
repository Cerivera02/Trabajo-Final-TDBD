...
--JOIN
SELECT A.nombre, M.nombre, A.id_carrera, C.nombreCarrera FROM ALUMNO AS A INNER JOIN CARRERA AS C
ON C.id_carrera=A.id_carrera INNER JOIN MAESTRO AS M ON M.id_carrera=C.id_carrera
...
CREATE VIEW VISTA3 AS SELECT C.nombreCarrera, M.nombre, M.id_carrera FROM 
CARRERA AS C INNER JOIN MAESTRO AS M ON C.id_carrera=M.id_carrera
select * FROM VISTA3
